...
import { X, LogIn, UserPlus, Building2, User, Mail, Lock, Phone, FileText, Sparkles, Image as ImageIcon, Upload } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (tab === 'login') {
        ...
        if (!name.trim()) throw new Error('El nombre del titular es obligatorio.');
        if (!orgName.trim()) throw new Error('El nombre de la organización o cuenta es obligatorio.');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">
                {tab === 'login' ? 'Iniciar Sesión' : 'Registrar Cuenta u Organización'}
              </h3>
              <p className="text-xs text-rose-800 dark:text-rose-300 font-medium">
                {tab === 'login' 
                  ? 'Accede a tu agenda y gestión de turnos privada' 
                  : 'Crea tu perfil con foto y gestiona tus propios turnos'}
              </p>
            </div>
          </div>
          <button
        ...
        {/* Tab switcher */}
        <div className="flex border-b border-rose-100 dark:border-rose-950 p-2 bg-rose-50/40 dark:bg-zinc-950">
          <button
            type="button"
            onClick={() => {
              setTab('login');
            ...
            className={`flex-1 py-2 text-xs font-bold rounded-2xl transition-all flex items-center justify-center gap-1.5 ${
              tab === 'login'
                ? 'bg-white dark:bg-zinc-900 text-rose-900 dark:text-white shadow-xs'
                    ...
                    <button
                      ...
                      className={`relative w-12 h-12 rounded-2xl overflow-hidden flex-shrink-0 border-2 transition-all ${
                        avatarUrl === av.url && !customAvatarInput
                          ? 'border-rose-600 ring-2 ring-rose-500/30 scale-105'
                          : 'border-rose-200 dark:border-rose-900 opacity-70 hover:opacity-100'
                      }`}
                      title={av.label}
                    >
